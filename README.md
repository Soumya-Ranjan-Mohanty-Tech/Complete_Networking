...
**NIC** Sends and receives frames, Stores the MAC address, Interacts directly with the physical network medium. 
**Switch**: Connects different devices within the same network.. **A switch forwards frames based on MAC addresses** **The switch learns which MAC address is connected    to which port and forwards frames accordingly.**  Smarter than "hub"- sends each message only to its target device. [Tracks MAC address to know which device is on which port]. Works on Layer 2 of the OSI model of Data Link Layer.  A HUB Broadcasts to everyone - a SWITCH is targetted and efficient. Security: MAC FLOODING forces a switch to act like a dumb hub. Attackers than see ALL TRAFFIC meant for everyone on that segment.
